...
5. 在 `scripts/config/board_profiles.json` 增加板配置
6. 跑构建与 smoke 回归
...
- `memory/myboard.x`
- `src/bsp/myboard.rs`
- （可选）更新 `scripts/config/board_profiles.json`
...
### 步骤 E：注册脚本板配置
编辑 `scripts/config/board_profiles.json`，新增一项：
- `name`
- `aliases`
- `feature`
- `target`
...
### 4.1 Compile-only
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/build/build_board.ps1 -Board myboard -Profile release -Mode app
```

### 4.2 烧录（多探针时建议指定 `-Probe`）
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/build/flash_board.ps1 -Board myboard -Image target/<target>/release/CortexOS -Probe <VID:PID[:SERIAL]> -ResetAfter
```

### 4.3 APP 烟雾
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/test/run_app_smoke.ps1 -Board myboard -Port COMx -Probe <VID:PID[:SERIAL]> -Flash
```

### 4.4 纳入多板回归
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/test/run_multiboard_regression.ps1 `
  -BuildMatrix @(
...
检查 `scripts/config/board_profiles.json` 是否新增并拼写一致（`name` 与 `aliases`）。

---

## 6. 变更约束
- 不要在板移植时修改 RTOS 核心语义（调度、IPC、同步）
- 板差异尽量收敛在 `src/bsp/*` 与 `scripts/config/board_profiles.json`
- 多板相关脚本改动后，至少执行一次：
