param(
    ...
    [int]$SmokeReadTimeoutMs = 3000,
    [int]$SmokeStartupWindowMs = 2500
)
...
$outDir = Join-Path 'regression_runs' $timestamp
    ...
    if ($smokeTargets.Count -eq 0) {
        ...
        foreach ($target in $smokeTargets) {
            ...
            $args = $prefix + @(
                ...
                '-Board', $target.Board,
                '-Port', $target.Port,
                '-ReadTimeoutMs', $SmokeReadTimeoutMs,
                '-StartupWindowMs', $SmokeStartupWindowMs
            )
            if ($target.Probe) {
