...
$outDir = Join-Path 'board_flash_runs' ("{0}_{1}" -f $timestamp, $boardConfig.name)
...
$proc = Start-Process -FilePath 'probe-rs' -ArgumentList $downloadArgs -WorkingDirectory (Get-Location) -NoNewWindow -Wait -PassThru -RedirectStandardOutput $stdoutPath -RedirectStandardError $stderrPath
...
if ($ResetAfter) {
    ...
    $resetProc = Start-Process -FilePath 'probe-rs' -ArgumentList $resetArgs -WorkingDirectory (Get-Location) -NoNewWindow -Wait -PassThru -RedirectStandardOutput $resetOut -RedirectStandardError $resetErr
