...
## 核心泛化能力
- 板配置统一来自 `scripts/config/board_profiles.json`
...
### 2) 双板 smoke（旧参数兼容）
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/test/run_multiboard_regression.ps1 -F103Port COM17 -F411Port COM6 -FlashOnSmoke false
```
...
## 输出
- `runs/regression/<timestamp>/meta.json`
- `runs/regression/<timestamp>/summary.csv`
- `runs/regression/<timestamp>/summary.json`
