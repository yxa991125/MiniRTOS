...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/build/build_board.ps1 -Board f103rct6-generic -Profile release -Mode uart-probe
```

## 输出
- `runs/build/<timestamp>_<board>_<profile>_<mode>/build.log`
- `runs/build/<timestamp>_<board>_<profile>_<mode>/build_meta.json`

## 说明
- 脚本会显式使用 `--no-default-features --features board-*`。
- `f103rct6-generic` 仅是脚本别名，便于在 `STM32F103RC*` 实板上复用同一 feature 进行验证。
- 板配置来自 `scripts/config/board_profiles.json`，新增板型时优先更新该文件。
