...
$outDir = Join-Path (Join-Path $repoRoot 'runs/build') ("{0}_{1}_{2}_{3}" -f $timestamp, $boardConfig.name, $Profile, $Mode)
...
$proc = Start-Process -FilePath 'cargo' -ArgumentList $cargoArgs -WorkingDirectory $repoRoot -NoNewWindow -Wait -PassThru -RedirectStandardOutput $stdoutPath -RedirectStandardError $stderrPath
