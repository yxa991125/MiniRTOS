...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/bench/collect_release_bench.ps1 -Port COM6
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/bench/collect_release_bench.ps1 -Port COM6 -Runs 10 -ReadTimeoutMs 180000
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/bench/collect_release_bench.ps1 -Port COM6 -Runs 30 -ReadTimeoutMs 180000 -NoFlash
```
...
脚本内部按下面顺序工作：
1. 检查 `-Port` 是否提供。
2. 创建本轮输出目录：`runs/bench/<timestamp>/`。
